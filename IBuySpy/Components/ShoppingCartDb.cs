...
namespace IBuySpy {
    ...
    public class ShoppingCartDB {
        ...
        private void CalcQuantityDiscounts(string cartId)
        {
            ProductsDB products = new ProductsDB();
            using (SqlDataReader result = products.GetQuantityDiscounts())
            {
                ...
                while (result.Read())
                {
                    int pId = result.GetInt32(0);
                    decimal quantityDiscount = Convert.ToDecimal(result.GetValue(1));
                    int discountLevel = result.GetInt32(2);

                    int cartQuantity = 0;
                    decimal cartUnitCost = 0.0M;
                    ...
                    using (SqlDataReader cart = shoppingCart.GetItems(cartId))
                    {
                        while (cart.Read())
                        {
                            int cartPId = cart.GetInt32(0);
                            cartQuantity = cart.GetInt32(3);
                            cartUnitCost = cart.GetDecimal(4);
                    ...
                    if (match)
                    {
                        if (cartQuantity > discountLevel)
                        {
                            decimal discountedUnitCost = cartUnitCost - (cartUnitCost * quantityDiscount);
                            if(!discountedUnitCost.Equals(cartUnitCost))
                            {
                                UpdateCartUnitCost(discountedUnitCost, matchPId, cartId);
                            }
                        ...
                        if (cartQuantity < discountLevel)
                        {
                            ...
                            ProductDetails details = products.GetProductDetails(matchPId);
                            UpdateCartUnitCost(details.UnitCost, matchPId, cartId);
                        }
        ...
        private void UpdateCartUnitCost(decimal cartUnitCost, int productId, string cartId)
        {
            // Create Instance of Connection and Command Object
            SqlConnection myConnection = new SqlConnection(ConfigurationSettings.AppSettings["ConnectionString"]);
            SqlCommand myCommand = new SqlCommand("UpdateQuantityDiscount", myConnection);
            ...
            SqlParameter parameterUnitCost = new SqlParameter("@UnitCost", SqlDbType.Money);
            parameterUnitCost.Value = cartUnitCost;
